...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.assethandover.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.assethandovers.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('empid') ? 'is-invalid' : '' }}" name="empid_id" id="empid_id" required>
                    @foreach($empids as $id => $empid)
                        <option value="{{ $id }}" {{ old('empid_id') == $id ? 'selected' : '' }}>{{ $empid }}</option>
                    @endforeach
                </select>
                @if($errors->has('empid'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('exitemailrec') ? 'is-invalid' : '' }}" name="exitemailrec" id="exitemailrec" required>
                    ...
                    @foreach(App\Assethandover::EXITEMAILREC_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('exitemailrec', 'NA') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('allassets') ? 'is-invalid' : '' }}" name="allassets" id="allassets" required>
                    ...
                    @foreach(App\Assethandover::ALLASSETS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('allassets', 'NA') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('addeactivationdate') ? 'is-invalid' : '' }}" type="text" name="addeactivationdate" id="addeactivationdate" value="{{ old('addeactivationdate') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('itapprovaldate') ? 'is-invalid' : '' }}" type="text" name="itapprovaldate" id="itapprovaldate" value="{{ old('itapprovaldate') }}">
